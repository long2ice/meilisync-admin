...
from meilisync_admin import auth
from meilisync_admin.auth import access_security, get_password_hash
from meilisync_admin.depends import get_current_admin, superuser_required
from meilisync_admin.models import Admin
...
async def create_admin(body: CreateAdminBody):
    try:
        ...
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail=i18n.t("admin_exists")
        )
...
async def change_password(
    body: ChangePasswordBody, admin: Admin = Depends(get_current_admin)
):
    if not auth.verify_password(body.old_password, admin.password):
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail=i18n.t("password_incorrect")
        )
